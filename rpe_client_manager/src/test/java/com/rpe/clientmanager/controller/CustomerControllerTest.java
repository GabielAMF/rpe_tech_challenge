...
class CustomerControllerTest {
    ...
    private static final String NEW_CUSTOMER = """
            {"name": "Maria Silva", "cpf": "123.456.789-09", "birthDate": "1990-05-20"}
            """;
    ...
    void getReturnsCustomer() throws Exception {
        when(customerService.findById(ID)).thenReturn(customer(ID));
    ...
    void postNormalizesCpfAndReturnsLocation() throws Exception {
        when(customerService.create("Maria Silva", new Cpf("12345678909"), BIRTH_DATE)).thenReturn(customer(ID));
    ...
    void postRejectsInvalidCpfWithoutEchoingIt() throws Exception {
        mockMvc.perform(post("/api/v1/customers").with(USER)
                        ...
                        .content("""
                                {"name": "Maria", "cpf": "98765", "birthDate": "1990-05-20"}
                                """))
    ...
    void postWithCpfOfCancelledCustomerReturns409WithCustomerId() throws Exception {
        when(customerService.create(anyString(), any(Cpf.class), any()))
