...
/**
 * AES-256-GCM encryption for sensitive card data at rest. Each value gets a random 12-byte IV, stored in front of
 * the ciphertext: {@code base64(iv || ciphertext || tag)}. GCM also authenticates, so a tampered value fails to
...
public class CardCipher {
    ...
    public CardCipher(CardProperties properties) {
        byte[] keyBytes = Base64.getDecoder().decode(properties.encryptionKey());
        if (keyBytes.length != 32) {
            throw new IllegalStateException("app.card.encryption-key must be base64 of exactly 32 bytes (AES-256)");
        }
    ...
    public String encrypt(String plaintext) {
        try {
            ...
            throw new IllegalStateException("Could not encrypt card data", ex);
        }
    }
    ...
    public String decrypt(String encrypted) {
        try {
            ...
            throw new IllegalStateException("Could not decrypt card data (wrong key or tampered value)", ex);
        }
    }
}
