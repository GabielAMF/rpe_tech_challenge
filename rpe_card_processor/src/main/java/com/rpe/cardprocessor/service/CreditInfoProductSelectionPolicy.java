...
/**
 * <b>PLACEHOLDER RULE</b> for {@link ProductSelectionPolicy}. There is no real credit analysis yet. For now {@code creditInfo} is read as a catalog
 * product id: if it is a UUID of a product that exists and is ATIVO, that product is used; otherwise (not a UUID,
 * unknown, or not ATIVO) the default product ({@code app.card.default-product-id}, GOLD) is used. When the business
 * defines how credit information maps to a product, add a new implementation and remove this one.
 */
@Slf4j
@Component
public class CreditInfoProductSelectionPolicy implements ProductSelectionPolicy {

    private final CatalogGateway catalog;
    private final UUID defaultProductId;

    public CreditInfoProductSelectionPolicy(CatalogGateway catalog, CardProperties properties) {
